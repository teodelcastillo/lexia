/**
 * Eventos Management Page
 *
 * Centralized view for all events and legal deadlines.
...
export const metadata = {
  title: 'Eventos',
  description: 'Gestión de eventos y plazos legales',
}
...
function getEventStatus(dueDate: string): {
  label: string
...
/** Maps event/deadline type to label; null/empty = Evento */
function getEventTypeLabel(type: string | null | undefined): string {
...
export default async function EventosPage({ searchParams }: EventosPageProps) {
  ...
  if (params.type && params.type !== 'all') {
    if (params.type === 'sin-tipo') {
      query = query.is('deadline_type', null)
    } else {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Controle los eventos y plazos de sus casos
            </p>
          </div>
              ...
              <TableBody>
                {deadlines && deadlines.length > 0 ? (
                  deadlines.map((deadline) => {
                    const status = getEventStatus(deadline.due_date)
                      ...
                      <TableRow key={deadline.id}>
                        ...
                        <TableCell>
                          <Badge variant="outline">
                            {getEventTypeLabel(deadline.deadline_type)}
                          </Badge>
                        </TableCell>
                        <TableCell>
