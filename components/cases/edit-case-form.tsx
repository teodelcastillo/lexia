...
import { Loader2, AlertCircle, Sparkles, ChevronDown, ChevronUp, ArrowLeft } from 'lucide-react'
...
export function EditCaseForm({
  ...
  const [formData, setFormData] = useState({
    ...
    case_type: initialCase.case_type || '',
    status: (initialCase.status || 'pending') as CaseStatus,
    company_id: initialCase.company_id || '',
    court_number: initialCase.court_number || '',
    jurisdiction: initialCase.jurisdiction || '',
    court_name: initialCase.court_name || '',
    opposing_party: initialCase.opposing_party || '',
    opposing_counsel: initialCase.opposing_counsel || '',
    filing_date: initialCase.filing_date ? initialCase.filing_date.split('T')[0] : '',
    brief_context: '',
  })
  ...
  useEffect(() => {
    ...
    const loadCompanies = async () => {
      ...
      try {
        ...
        console.error('[EditCaseForm] Error loading companies:', err)
  ...
  const handleGenerateDescription = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Error al generar la descripción'
      )
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Error al actualizar caso'
      setError(message)
      console.error('[EditCaseForm] Update error:', err)
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Modifique los datos del caso legal
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Actualice los detalles del caso
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger id="company_id">
                    <SelectValue
                      placeholder={
                        isLoadingCompanies ? 'Cargando empresas...' : 'Seleccionar empresa'
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <div className="grid gap-4 sm:grid-cols-2 pt-4 space-y-4">
                  ...
                  <div className="space-y-2 sm:col-span-2">
                    <label htmlFor="brief_context" className="text-sm font-medium">
                      Contexto adicional (para generar descripción)
                    </label>
                    <Textarea
                      id="brief_context"
            ...
            <div className="flex gap-3 pt-4">
              ...
              <Button type="button" variant="outline" asChild disabled={isLoading}>
                <Link href={`/casos/${caseId}`}>Cancelar</Link>
              </Button>
            </div>
          </form>
        </CardContent>
